...
async function fetchFromSupabase(slug: string): Promise<BlogPostDetail | null> {
  try {
    ...
    return {
      ...
      author: {
        name: data.author_name || 'ACI Team',
        title: data.author_title || 'ACI Infotech',
        bio: data.author_bio || '',
        image: data.author_image_url || undefined,
        linkedin: data.author_linkedin || undefined,
      },
      category: data.category || 'Insights',
      tags: data.tags || [],
      featured_image: data.featured_image_url || data.featured_image || undefined,
      read_time: `${data.read_time_minutes || 5} min read`,
...
export default async function BlogPostPage({ params }: PageProps) {
  const { slug } = await params;
  const post = blogPostsData[slug] ?? await fetchFromSupabase(slug);
